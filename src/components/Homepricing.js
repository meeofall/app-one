import React from 'react'

function Pricing() {
    return (
        <>
            <div class="container container-fluid pricing justify-content-center mb-5">
          <h2 class="pricing-h2 text-center mb-5 mt-3">Pricing</h2>
  
          <div class="container justify-content-center">
    <div class="row">
       <div class="col-md-6 col-lg-3">
        <div class="card-p mb-5 mb-lg-0">
          <div class="card-body justify-content-center">
            <h5 class="card-title glassy202 text-muted text-uppercase text-center">Basic</h5>
            <h6 class="card-price text-center"><span class="double-strike"><div class="the-lines"></div>N</span>10,000<span class="period">/ month</span></h6>
            <hr />
            <ul class="fa-ul">
              <li><span class="fa-li"><i class="fas fa-check"></i></span><strong>1-100 Students</strong></li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>50GB Storage</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>Complete Access</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>Email Support</li>
              <li class="text-muted"><span class="fa-li"><i class="fas fa-times"></i></span>Free Website Sync</li>
              <li class="text-muted"><span class="fa-li"><i class="fas fa-times"></i></span>Dedicated Phone Support</li>
              <li class="text-muted"><span class="fa-li"><i class="fas fa-times"></i></span>24/7 Support</li>
            </ul>
            <a href="https://clients.inovateos.com/signup" class="btn btn-block btn-primary text-uppercase">Get Started</a>
          </div>
        </div>
              </div>
              

      <div class="col-md-6 col-lg-3">
        <div class="card-p mb-5 mb-lg-0">
          <div class="card-body">
            <h5 class="card-title glassy202 text-muted text-uppercase text-center">Standard</h5>
            <h6 class="card-price text-center"><span class="double-strike"><div class="the-lines"></div>N</span>17,500<span class="period">/ month</span></h6>
            <hr />
            <ul class="fa-ul">
              <li><span class="fa-li"><i class="fas fa-check"></i></span><strong>101-200 Students</strong></li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>100GB Storage</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>Complete Access</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>Email Support</li>
              <li class="text-muted"><span class="fa-li"><i class="fas fa-times"></i></span>Free Website Sync</li>
              <li class="text-muted"><span class="fa-li"><i class="fas fa-times"></i></span>Dedicated Phone Support</li>
              <li class="text-muted"><span class="fa-li"><i class="fas fa-times"></i></span>24/7 Support</li>
            </ul>
            <a href="https://clients.inovateos.com/signup" class="btn btn-block btn-primary text-uppercase">Get Started</a>
          </div>
        </div>
      </div>
       
      <div class="col-md-6 col-lg-3">
        <div class="card-p mb-5 mb-lg-0">
          <div class="card-body">
            <h5 class="card-title glassy202 text-muted text-uppercase text-center">Premium</h5>
            <h6 class="card-price text-center"><span class="double-strike"><div class="the-lines"></div>N</span>25,000<span class="period">/ month</span></h6>
            <hr />
            <ul class="fa-ul">
              <li><span class="fa-li"><i class="fas fa-check"></i></span><strong>201-300 Students</strong></li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>200GB Storage</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>Complete Access</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>Email Support</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span> Free Website Sync</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>24/7 Support</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>Dedicated Phone Support</li>
            </ul>
            <a href="https://clients.inovateos.com/signup" class="btn btn-block btn-primary text-uppercase">Get Started</a>
          </div>
        </div>
        </div>
                        
        <div class="col-md-6 col-lg-3">
        <div class="card-p mb-5 mb-lg-0">
          <div class="card-body">
            <h5 class="card-title glassy202 text-muted text-uppercase text-center">Premium Pro</h5>
            <h6 class="card-price text-center"><span class="double-strike"><div class="the-lines"></div>N</span>45,000<span class="period">/ month</span></h6>
            <hr />
            <ul class="fa-ul">
              <li><span class="fa-li"><i class="fas fa-check"></i></span><strong>Over 300 Students</strong></li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>500+GB Storage</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>Complete Access</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>Email Support</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span> Free Website Sync</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>24/7 Support</li>
              <li><span class="fa-li"><i class="fas fa-check"></i></span>Dedicated Phone Support</li>
            </ul>
            <a href="https://clients.inovateos.com/signup" class="btn btn-block btn-primary text-uppercase">Get Started</a>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>
        </>
    )
}

export default Pricing
